...
class ImageInputComponent {
  ...
  // Event Handlers
  private readonly _change = (e: Event) => {
    e.preventDefault();

    if (this.inputElement !== null && this.inputElement.files && this.inputElement.files[0]) {
      // Fire change event
      if (!EventHandlerUtil.trigger(this.element, 'kt.imageinput.change', e)) {
        return;
      }
  ...
  private readonly _cancel = (e: Event) => {
    e.preventDefault();

    // Fire cancel event
    if (!EventHandlerUtil.trigger(this.element, 'kt.imageinput.cancel', e)) {
      return;
    }
  ...
  private readonly _remove = (e: Event) => {
    e.preventDefault();

    // Fire remove event
    if (!EventHandlerUtil.trigger(this.element, 'kt.imageinput.remove', e)) {
      return;
    }
