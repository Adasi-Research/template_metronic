...
class SearchComponent {
  ...
  private readonly _getElement = (name: string) => {
    return this.element.querySelector('[data-kt-search-element="' + name + '"]');
  };

  // Get option
  private readonly getOption = (name: string) => {
  ...
  private readonly getResponsiveFormMode = () => {
  ...
  // Enter
  private readonly enter = (e: KeyboardEvent) => {
  ...
  // Clear
  public clear = () => {
    if (!EventHandlerUtil.trigger(this.element, 'kt.search.clear')) {
      return;
    }
