...
class DrawerComponent {
  ...
  private readonly _getOption = (name: string) => {
  ...
  private readonly _toggle = () => {
    if (!EventHandlerUtil.trigger(this.element, 'kt.drawer.toggle')) {
      return
    }
  ...
  private readonly _hide = () => {
    if (!EventHandlerUtil.trigger(this.element, 'kt.drawer.hide')) {
      return
    }
  ...
  private readonly _createOverlay = () => {
  ...
  public off = (name: string, handerId: string) => {
    EventHandlerUtil.off(this.element, name, handerId)
  }
