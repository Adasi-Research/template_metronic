...
class MenuComponent {
  ...
  private readonly _setTriggerElement = () => {
  ...
  private readonly _isTriggerElement = (item: HTMLElement) => {
    return this.triggerElement === item;
  };

  // Get item option(through html attributes)
  private readonly _getItemOption = (item: HTMLElement, name: string) => {
  ...
  // Get item element
  private readonly _getItemElement = (_element: HTMLElement): HTMLElement | undefined => {
  ...
  private readonly _getItemParentElement = (item: HTMLElement) => {
  ...
  private readonly _getItemParentElements = (item: HTMLElement) => {
  ...
  private readonly _getDropdownPopperConfig = (item: HTMLElement) => {
    // Placement
  ...
  private readonly _getItemChildElement = (item: HTMLElement): HTMLElement | null => {
    let selector = item;
  ...
  private readonly _getItemChildElements = (item: HTMLElement) => {
  ...
  private readonly _getItemSubElement = (item: HTMLElement): HTMLElement | null => {
    if (!item) {
      return null;
    }
  ...
  private readonly _getCss = (el: HTMLElement, styleProp: string) => {
  ...
  // Get item sub type
  private readonly _getItemSubType = (element: HTMLElement) => {
  ...
  private readonly _isItemSubShown = (item: HTMLElement) => {
  ...
  private readonly _isItemDropdownPermanent = (item: HTMLElement) => {
    return this._getItemOption(item, 'permanent') === true;
  };

  // Test if item's parent is shown
  private readonly _isItemParentShown = (item: HTMLElement) => {
    return getElementParents(item, '.menu-item.show').length > 0;
  };

  // Test of it is item sub element
  private readonly _isItemSubElement = (item: HTMLElement) => {
    return item.classList.contains('menu-sub');
  };

  // Test if item has sub
  private readonly _hasItemSub = (item: HTMLElement) => {
    return item.classList.contains('menu-item') && item.hasAttribute('data-kt-menu-trigger');
  };

  // Get link element
  private readonly _getItemLinkElement = (item: HTMLElement) => {
    return getElementChild(item, '.menu-link');
  };

  // Get toggle element
  private readonly _getItemToggleElement = (item: HTMLElement) => {
    if (this.triggerElement) {
  ...
  // Show item dropdown
  private readonly _showDropdown = (item: HTMLElement) => {
  ...
  private readonly initDropdownPopper = (item: HTMLElement, sub: HTMLElement) => {
  ...
  // Hide item dropdown
  private readonly _hideDropdown = (item: HTMLElement) => {
  ...
  private readonly destroyDropdownPopper = (item: HTMLElement) => {
    if (DataUtil.has(item, 'popper')) {
      // @ts-expect-error
      DataUtil.get(item, 'popper').destroy();
      DataUtil.remove(item, 'popper');
  ...
  private readonly _showAccordion = (item: HTMLElement) => {
  ...
  private readonly _hideAccordion = (item: HTMLElement) => {
  ...
  private readonly _hideAccordions = (item: HTMLElement) => {
  ...
  private readonly _reset = (item: HTMLElement) => {
  ...
  private readonly _update = () => {
    ...
    items.forEach((el) => {
      this._reset(el as HTMLElement);
    });
  };

  // Hide item sub
  private readonly _hide = (item: HTMLElement) => {
    if (!item) {
      return;
    }
  ...
  // Show item sub
  private readonly _show = (item: HTMLElement) => {
    if (!item) {
      return;
    }
  ...
  // Toggle item sub
  private readonly _toggle = (item: HTMLElement) => {
    if (!item) {
      return;
    }
  ...
  // Mouseout handle
  private readonly _mouseout = (element: HTMLElement, e: MouseEvent) => {
  ...
  // Mouseover handle
  private readonly _mouseover = (element: HTMLElement, e: MouseEvent) => {
    const item = this._getItemElement(element);
    if (!item) {
      return;
    }
  ...
  // Dismiss handler
  private readonly _dismiss = (element: HTMLElement, e: Event) => {
    const item = this._getItemElement(element);
    if (!item) {
      return;
    }
  ...
  // Link handler
  private readonly _link = (element: HTMLElement, e: Event) => {
  ...
  private readonly _click = (element: HTMLElement, e: Event) => {
    e.preventDefault();
    const item = this._getItemElement(element);
    if (item) {
  ...
  public mouseover = (element: HTMLElement, e: Event) => {
    this._mouseover(element, e as MouseEvent);
  };

  public mouseout = (element: HTMLElement, e: Event) => {
    this._mouseout(element, e as MouseEvent);
  };
  ...
  public on = (name: string, handler: any) => {
    EventHandlerUtil.on(this.element, name, handler);
  };
  ...
  public off = (name: string, handlerId: string) => {
    EventHandlerUtil.off(this.element, name, handlerId);
  };
